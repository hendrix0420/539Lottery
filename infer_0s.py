...
def get_previous_date(date, lottery_data):    
    ...
    try:
        ...
        if index > 0:
            previous_date = lottery_data[['日期']].iloc[index - 1].values[0]
    except IndexError:        
        # 異常發生時不做任何動作，直接跳過
        pass
...
# 打開 CSV 檔案以寫入模式
# 如果檔案已經存在，先將其清空
if os.path.exists('infer_0s_results.csv'):
    with open('infer_0s_results.csv', 'w', newline='', encoding='utf-8-sig') as f:
        f.truncate()

# 開啟 CSV 檔案以附加模式
def save_csv(specified_date, specified_date_numbers, top_numbers, matched_numbers):    
    with open('infer_0s_results.csv', 'a', newline='', encoding='utf-8-sig') as csvfile:
        # 定義 CSV 欄位名稱
...
while start_date_index is not None and start_date_index < len(date):      
    ...
    # 比對指定日期的開獎號碼
    def check_prediction(specified_date, top_numbers, date, drawings):
    ...
    specified_date_numbers = check_prediction(specified_date, top_numbers, date, drawings)
    if specified_date_numbers is not None:
        matched_numbers = set(top_numbers).intersection(set(specified_date_numbers))
        print(f"預測號碼與指定日期 {specified_date} 開獎號碼對中的號碼數量為: {len(matched_numbers)}")
    ...
    # 更新指定日期前一期的日期及開獎號碼
    pecified_date = date[start_date_index]
...
print("結果已保存至 infer_0s_result.csv")