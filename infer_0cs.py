...
# 打開 CSV 檔案以寫入模式
if os.path.exists('infer_0cs_results.csv'):
    with open('infer_0cs_results.csv', 'w', newline='', encoding='utf-8-sig') as f:
        f.truncate()

# 開啟 CSV 檔案以附加模式
def save_csv(current_date, current_numbers, predicted_numbers, matched_numbers):    
    with open('infer_0cs_results.csv', 'a', newline='', encoding='utf-8-sig') as csvfile:
...
def update_features(previous_features, current_numbers):
    new_features = previous_features.copy()
    
    # 更新號碼出現次數特徵
    for num in current_numbers:
        if 1 <= num <= 39:
            new_features[num-1] += 1
    
    # 更新其他特徵
    # 這裡需要根據您的實際特徵計算方法來完善
    # 例如，更新連號特徵、雙生組合特徵、三生組合特徵和斷龍特徵等
    
    return new_features

def predict_next_numbers(model, previous_features, previous_numbers, window_size, top_n):
    last_feature = previous_features.astype(np.float64)  # 將特徵轉換為浮點數類型
    ...
    # 添加一些隨機噪聲來增加變化性
    noise = np.random.normal(0, 0.01, last_feature.shape)
    last_feature += noise
    
    # 計算 windowed_feature
    windowed_feature = np.mean(features[-window_size:], axis=0).astype(np.float64)
    windowed_feature = np.expand_dims(windowed_feature, axis=0)
    
    input_features = np.concatenate((last_feature, windowed_feature, last_vector), axis=1)

    # 確保輸入特徵的形狀為 (1, 103)
    if input_features.shape[1] < 103:
        padding = np.zeros((1, 103 - input_features.shape[1]))
        input_features = np.concatenate((input_features, padding), axis=1)
    elif input_features.shape[1] > 103:
        input_features = input_features[:, :103]

    predicted_probs = model.predict(input_features)[0]
    
    predicted_probs += np.random.normal(0, 0.01, predicted_probs.shape)  # 添加一些隨機性
    predicted_probs = np.clip(predicted_probs, 0, None)  # 確保概率非負
...
if start_date_index is None:
    ...
    for i in range(start_date_index, len(date)):
        ...
        if i > start_date_index:
            ...
            previous_numbers = drawings[i-1]
        else:
            previous_features = features[i]
            previous_numbers = drawings[i]
        
        print(f"正在預測日期 {current_date} 的開獎號碼")
        
        predicted_numbers = predict_next_numbers(model, previous_features, previous_numbers, window_size, top_n)
        print(f"預測本期539開獎號碼範圍為: {predicted_numbers}")
        
        matched_numbers = set(predicted_numbers).intersection(set(current_numbers))
        print(f"實際開獎號碼: {current_numbers}")
        ...
        # 更新特徵
        if i < len(features) - 1:
            features[i+1] = update_features(features[i], current_numbers)
        elif i == len(features) - 1:
            # 如果已經到達 features 的末尾，添加新的特徵
            new_feature = update_features(features[i], current_numbers)
...
print("結果已保存至 infer_0cs_results.csv")
