...
for draw in drawings:
    feature = np.bincount(draw, minlength=40)  # 注意这里将 minlength 设置为40，因为最大号码是39
...
# 将标签编码为一个热向量
y = np.array([to_categorical(label, num_classes=40) for label in y])
...
model = Sequential([
    ...
    Dense(40, activation='softmax')  # 输出维度为40,每个号码是一个40分类问题
])
...
def to_vector(numbers):
    vector = np.zeros(5)  # 将向量长度改为5
    for num in numbers:
        if num <= 5:  # 限制号码范围在1到5之间
            vector[num-1] = 1
    return vector

# 將最後一期開獎號碼向量化，使其長度為 5
...
def predict_next_numbers(model, features, drawings, window_size):
    ...
    predicted_numbers = [i+1 for i, prob in enumerate(predicted_probs.flatten()) if prob > 0.5]
    # predicted_numbers = [num for num in predicted_numbers if num <= 39]
